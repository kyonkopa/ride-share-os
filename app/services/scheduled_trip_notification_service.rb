...
class ScheduledTripNotifier
  ...
  def self.acceptance_url(scheduled_trip)
    ...
    port_str = port ? ":#{port}" : ""
    "#{protocol}://#{base_url}#{port_str}/scheduled_trips/#{scheduled_trip.acceptance_token}/accept"
  end
  ...
  def self.decline_url(scheduled_trip)
    ...
    protocol = Rails.env.production? ? "https" : "http"
    port_str = port ? ":#{port}" : ""
    "#{protocol}://#{base_url}#{port_str}/scheduled_trips/#{scheduled_trip.decline_token}/decline"
  end
end
