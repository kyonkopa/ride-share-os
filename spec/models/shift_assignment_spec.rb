...
RSpec.describe ShiftAssignment do
  ...
  describe '#actual_start_time' do
    ...
    context 'when there is a clock_in event' do
      let!(:clock_in_event) { create(:shift_event, :clock_in, shift_assignment:) }
    ...
    context 'when there are multiple events but no clock_in' do
      before do
        create(:shift_event, :telemetry_snapshot, shift_assignment:)
    ...
    context 'when there are multiple clock_in events' do
      let!(:first_clock_in) { create(:shift_event, :clock_in, shift_assignment:, created_at: 2.hours.ago) }
  ...
  describe '#actual_end_time' do
    ...
    context 'when there is a clock_out event' do
      let!(:clock_out_event) { create(:shift_event, :clock_out, shift_assignment:) }
    ...
    context 'when there are multiple clock_out events' do
      let!(:first_clock_out) { create(:shift_event, :clock_out, shift_assignment:, created_at: 2.hours.ago) }

      before do
        create(:shift_event, :clock_out, shift_assignment:, created_at: 1.hour.ago)
      end
  ...
  describe 'integration with shift events' do
    ...
    context 'when a shift has both clock_in and clock_out events' do
      let!(:clock_in_event) { create(:shift_event, :clock_in, shift_assignment:, created_at: 2.hours.ago) }
      let!(:clock_out_event) { create(:shift_event, :clock_out, shift_assignment:, created_at: 1.hour.ago) }
